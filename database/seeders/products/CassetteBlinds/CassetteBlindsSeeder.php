...
class CassetteBlindsSeeder extends Seeder
{
    public function run(): void
    {
        $productTitle = 'Rolety w kasecie';
        ...
        UploadFileFromSeeder::upload($product, 'roleta-w-kasecie.jpg','cover');
        ...
        $dimensions = AttributeValue::query()
            ...
            ->whereIn('name', ['pomiar_a', 'pomiar_b', 'ilosc'])
            ->get();
        ...
        foreach ($pricesListData as $entry) {
            ...
            if (isset($entry['conditions']['collections']) && is_array($entry['conditions']['collections'])) {
                foreach ($entry['conditions']['collections'] as $data) {
                    ...
                    $itemId = ProductCollectionItem::where('name', $data['collection_item'])->value('id');
